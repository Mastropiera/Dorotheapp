...
const mifflinStJeorSchema = z.object({
...
type MifflinStJeorFormValues = z.infer<typeof mifflinStJeorSchema>;

const MifflinStJeorCalculator: React.FC = () => {
  const [gebResult, setGebResult] = useState<number | null>(null);

  const form = useForm<MifflinStJeorFormValues>({
    resolver: zodResolver(mifflinStJeorSchema),
    defaultValues: {
      peso: undefined,
      talla: undefined,
      edad: undefined,
  ...
  const onSubmit = (data: MifflinStJeorFormValues) => {
    let bmr = 0;
    if (data.sexo === 'masculino') {
      bmr = (10 * data.peso) + (6.25 * data.talla) - (5 * data.edad) + 5;
    } else { // femenino
      bmr = (10 * data.peso) + (6.25 * data.talla) - (5 * data.edad) - 161;
    }
    setGebResult(bmr);
  };
  ...
  const renderInputField = (name: keyof Pick<MifflinStJeorFormValues, "peso" | "talla" | "edad">, label: string, placeholder: string, unit: string) => (
    <FormField
  ...
  return (
    <Card className="w-full max-w-lg mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center"><Flame className="mr-2 h-6 w-6 text-orange-500" />Fórmula de Mifflin-St Jeor</CardTitle>
        <CardDescription>
          Calcula el Gasto Energético Basal (GEB) o Basal Metabolic Rate (BMR). Considerada más precisa que Harris-Benedict por algunas organizaciones.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
        ...
        <div className="text-xs text-muted-foreground space-y-2">
            <p className="flex items-start"><Info size={24} className="mr-2 flex-shrink-0 text-blue-500"/> 
                La fórmula de Mifflin-St Jeor es otra ecuación predictiva para el metabolismo basal.
            </p>
            <p><strong>Fórmulas:</strong></p>
            <ul className="list-disc list-inside pl-4 space-y-1">
                <li><strong>Hombres:</strong> GEB = (10 × P) + (6.25 × T) - (5 × E) + 5</li>
                <li><strong>Mujeres:</strong> GEB = (10 × P) + (6.25 × T) - (5 × E) - 161</li>
            </ul>
            <p>(P: Peso en kg, T: Talla en cm, E: Edad en años)</p>
            <p className="italic">
                Esta fórmula es a menudo preferida por su mayor precisión en la población general adulta en comparación con Harris-Benedict. Al igual que con otras fórmulas, el GEB debe ajustarse por factores de actividad y estrés.
            </p>
        </div>
      </CardContent>
    </Card>
