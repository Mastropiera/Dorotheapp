...
const DVAMidazolamCalculator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const concentracionMgMl = (drogaMg || 0) / (volumenMl || 1);

    if (calculating === 'rate') {
        if (peso && indicacionDosis !== undefined && concentracionMgMl > 0) {
            const mlh = (indicacionDosis * peso) / concentracionMgMl;
    ...
    if (calculating === 'dose') {
        if (peso && velocidadInfusion !== undefined && peso > 0) {
            const dosis = (velocidadInfusion * concentracionMgMl) / peso;
            setValue('indicacionDosis', parseFloat(dosis.toFixed(3)));
        }
    }
  ...
  const concentracion = (watchedValues.drogaMg || 0) / (watchedValues.volumenMl || 1);

  return (
    <Card className="w-full max-w-xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center"><Syringe className="mr-2 h-6 w-6 text-cyan-500" />Cálculo de DVA: Midazolam</CardTitle>
        <CardDescription>
          Benzodiacepina de acción corta. Produce sedación, amnesia anterógrada y ansiolisis.
        </CardDescription>
        <p className="text-xs text-muted-foreground pt-1">Presentación: Ampolla de 5mg/1ml, 15mg/3ml, 50mg/10ml</p>
        <p className="text-xs text-muted-foreground pt-1">Dilución estándar: 100mg en 100ml de SF 0,9%/SG 5% = 1mg/ml</p>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form className="space-y-6">
            <Card className="p-4 bg-muted/30">
                ...
                <CardContent className="p-0 grid grid-cols-2 gap-4">
                    {renderInputField("drogaMg", "Fármaco (Midazolam)", "mg")}
                    ...
                    <div className="col-span-2 text-sm font-medium text-center p-2 bg-background rounded-md">
                        Concentración: {concentracion.toFixed(2)} mg/ml
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="space-y-4">
                ...
                <div className="p-4 border rounded-lg shadow-sm">
                    ...
                    {renderInputField("indicacionDosis", "Dosis Indicada", "mg/kg/h", <Syringe />)}
                ...
                <div className="p-4 border rounded-lg shadow-sm">
                    ...
                    <Button type="button" onClick={() => handleCalculate('dose')} className="mt-2 w-full">Calcular mg/kg/h</Button>
                </div>
            </div>
        ...
        <div className="text-xs text-muted-foreground space-y-2">
            <p className="flex items-start"><Info size={24} className="mr-2 flex-shrink-0 text-blue-500"/>
                Esta calculadora utiliza las fórmulas estándar para la infusión de sedantes.
            </p>
            <p><strong>Fórmulas:</strong></p>
            <ul className="list-disc list-inside pl-4">
                <li>ml/h = (Dosis [mg/kg/h] × Peso [kg]) / Concentración [mg/ml]</li>
                <li>Dosis [mg/kg/h] = (Velocidad [ml/h] × Concentración [mg/ml]) / Peso [kg]</li>
            </ul>
             <p className="italic">
                Verifique siempre los cálculos y protocolos institucionales. La infusión de Midazolam requiere monitorización continua.
            </p>
        </div>
      </CardContent>
    </Card>
...
export default DVAMidazolamCalculator;